...
ScholarSkills 正式承接全部十个模块的通用学术能力 source / contract / docs：`lit`、`tables`、`stats`、`submit`、`write`、`review`、`omics`、`data`、`intake` 与 `display` 同级进入本仓 contract snapshot、Skill 入口和人读说明。这个承接只限 refs-only / no-authority handoff。所有模块都必须能说明 `source_pack_ref`、`candidate_package_ref`、`execution_receipt_ref` 和 `owner_gate_handoff_ref`，但这些 ref 只描述候选输入、候选包、未签执行回执和下游 owner gate 交接点，不写 MAS domain truth、owner receipt、typed blocker、human gate、publication readiness authority、runtime queue 或 provider attempt。
...
FeedbackOps adapter 同样保持 refs-only：`feedbackops_refs_only_adapter_policy` 把 ScholarSkills 声明为 OPL FeedbackOps 的 capability adapter，profile 为 `target_agent_feedback_external_suite`。它可以从 `delivery_feedback_ref` / `feedbackops_intake_ref` 生成 `candidate_refs`、quality hints、Display/Write/Review capability suggestions、`route_back_candidate_ref` 和 `stop_or_continue_recommendation_ref`，供 MAS 或 OMA 作为 evidence input 消费。MAS/OMA 消费后若要签 `owner_receipt_ref`、产出 typed blocker、质量 verdict、artifact mutation 或 current-package update，必须回到各自 owner surface；ScholarSkills 不能写 MAS/current_package，也不能声明 quality verdict、owner acceptance、typed blocker 或 publication readiness。
