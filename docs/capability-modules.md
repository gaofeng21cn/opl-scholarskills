...
Display 的 quality floor 现在明确区分数据证据图、页面级组合图和 graphical abstract / illustration shell。数据证据图仍优先消费 MAS Display Pack 的 R/ggplot2 证据模板与 paper-local visual audit；graphical abstract 不应复用 gallery 中单个 `submission_graphical_abstract` 作为最终模板。它应走 `brief_first_reference_guided_ai_candidate_not_single_template_reuse`：先锁定核心主张、证据链、figure contract、参考风格和 reviewer rubric，再由 AI 执行者自由选择合适图型、布局、面板层级、backend 和候选数量，最后提供 critic review、final-size inspection 和 domain owner gate refs。这样提高质量下限，但不把 ScholarSkills 变成质量 verdict、artifact authority 或 publication-ready owner。
